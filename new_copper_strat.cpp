...
static FuturesSeries load_futures(const std::string& path) {
    ...
    while (std::getline(f, line)) {
        ...
        try {
            ...
            // Raw prices stored as-is; unit conversions are applied downstream
            // at the point of use (see Cu/Gold ratio block for the HG cents/lb
            // conversion that matches the spec formula).
            bar.open = raw_open;
            bar.high = raw_high;
            bar.low = raw_low;
            bar.close = raw_close;
    ...
    std::vector<DailySignal> run() {
        ...
        for (int i = 0; i < n; ++i) {
            if (!std::isnan(hg[i]) && !std::isnan(gc[i]) && gc[i] > 0.0) {
                // Spec Layer 1 (Notional Normalization):
                //   Cu_notional = HG_price * 25000 / 100   (HG_price in cents/lb)
                //   Au_notional = GC_price * 100
                // Data file delivers HG in $/lb, so convert to cents/lb first,
                // then apply the spec formula verbatim.
                double hg_cents = hg[i] * 100.0;
                double cu_notional = hg_cents * 25000.0 / 100.0;
...
int main(int argc, char* argv[]) {
    ...
    std::string data_dir = "/Users/jonahnissan/CLionProjects/trade-engine/apps/backtest/Gannon_copper_Ip/data 2/raw";
    double initial_capital = 1000000.0;
